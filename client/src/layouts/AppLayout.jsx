...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-screen lg:flex">
      <aside className="w-full bg-ink text-white lg:min-h-screen lg:w-64">
        ...
        <nav className="space-y-1 p-3">
          {links.filter(([, , , ownerOnly]) => !ownerOnly || user?.role === "owner").map(([to, label, Icon]) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm ${isActive ? "bg-white text-ink" : "text-white/70 hover:bg-white/10"}`
              }
            >
              <Icon size={18} />
              {label}
      ...
      <main className="min-w-0 flex-1">
        <header className="flex h-16 items-center justify-end border-b bg-white px-8">
          <span className="mr-4 text-sm font-semibold">{user?.name}</span>
          <button
            onClick={async () => {
              await logout();
              nav("/login");
